...
def update_ema_variables_local(model, ema_model, alpha, global_step):
    alpha = 0.05
    for ema_param, param in zip(ema_model.parameters(), model.parameters()):
        ema_param.data.mul_(alpha).add_(1 - alpha, param.data)  
...
def main(device, args):
    ...
    for iter in range(args.num_epochs):
        ...
        for batch_idx, ((img, img_ema), label) in enumerate(train_loader_labeled):   
                ...
                update_ema_variables(model_glob, model_glob_ema, args.ema_decay, iter*len(train_loader_labeled) + batch_idx)
        ...
        for idx in idxs_users:
            ...
            for i, ((images1, images2), labels) in enumerate(train_loader_unlabeled):
                ...
                loss.backward()
                optimizer.step()
                update_ema_variables(model_glob, model_glob_ema, 0.99, user_epoch[idx]*len(train_loader_unlabeled) + batch_idx)
