...
from tools.ramps import get_median
...
def quantile_linear(iter, args):
    ...
    turn_point = int( (args.comu_rate * args.num_epochs - 0.1 * args.num_epochs -1.35) / 0.45 )
    if iter < args.phi_g:
        return 1.0
...
def main(device, args):
    ...
    if torch.cuda.device_count() > 1: model_glob_ema = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model_glob_ema)
    ...
    for iter in range(args.num_epochs):
        ...
        for batch_idx, ((img, img_ema), label) in enumerate(train_loader_labeled):   
                ...
                update_ema_variables(model_glob, model_glob_ema, 0.99, iter*len(train_loader_labeled) + batch_idx)
        ...
        if iter%5==0:
            ...
            model_glob_ema.eval()
            accuracy_ema, loss_train_test_labeled = test_img(model_glob_ema, test_loader, args)
            del test_loader
            gc.collect()
            torch.cuda.empty_cache()
        ...
        for idx in idxs_users:
            ...
            for i, ((images1, images2), labels) in enumerate(train_loader_unlabeled):
                ...
                labeled_minibatch_size = target_var.data.ne(-1).sum()    
                with torch.no_grad():
                    ema_model_out = model_local_ema(ema_input_var)
                ...
                if iter > args.phi_g:
                    update_ema_variables(model_local, model_local_ema, args.ema_decay, user_epoch[idx]*len(train_loader_unlabeled) + batch_idx )
                else:
                    update_ema_variables(model_local, model_local_ema, 0.0, user_epoch[idx]*len(train_loader_unlabeled) + batch_idx )
                
            w, w_ema = model_glob.state_dict(), model_glob_ema.state_dict()
            ...
            for i in w_keys:
                if(iter < args.phi_g):
                    ...
                    if diff_w_ema[i] >= diff_w_old:
                        w_dic[i] = w[i]
                        comu_w += torch.numel(w_dic[i])
                    else:
                        w_ema_dic[i] = w_ema[i]
                        comu_w_ema += torch.numel(w_ema_dic[i])
#             return w_dic, w_ema_dic, w_ema, diff_w_ema 


            for i in list(w_ema.keys()):
                diff_w_old_dic.append(diff_w_ema[i])

            net_glob_new = get_model('fedfixmatch', args.backbone).to(device)
            ...
            w_locals_ema.append(copy.deepcopy(w_ema))
